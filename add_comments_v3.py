...
for f in sorted(files):
    ...
    # The statement might look like $table->string('name')->...
    # We can match `$table->type(args)` and see if the *entire statement* until `;` has `comment(`.

    # We use regex that matches $table-> followed by method and args
    # and then grabs everything until the next semicolon
    ...
    def repl_stmt(m):
        ...
        # We need to insert ->comment('...') right after base_call
        # e.g., $table->string('name')->comment('...')
