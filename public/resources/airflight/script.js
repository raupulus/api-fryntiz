...
function fetchData() {
	...
	FetchPending = $.ajax({ url: urlAircrafts,
                                timeout: 5000,
                                cache: false,
                                dataType: 'json' });
...
function reaper() {
	...
	// Look for planes where we have seen no messages for >historyTimeToDelete seconds
        var newPlanes = [];
        for (var i = 0; i < PlanesOrdered.length; ++i) {
                var plane = PlanesOrdered[i];
