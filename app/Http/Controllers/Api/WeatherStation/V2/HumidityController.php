...
/**
 * Controlador de datos de humedad para API V2.
 */
class HumidityController extends BaseApiController
{
    public function __construct(private WeatherStationService $service) {}

    /**
     * Lista datos de humedad con filtro opcional por rango de fechas.
     */
    public function index(Request $request): JsonResponse
    {
        $data = $this->service->getPreparedData('humidity', $request->query('from'), $request->query('to'));

        return $this->successResponse(HumidityResource::collection($data));
    }

    /**
     * Almacena dato de humedad.
     */
    public function store(StoreSensorRequest $request): JsonResponse
    {
        $record = $this->service->storeGenericData(
            ['humidity' => [$request->validated()]],
