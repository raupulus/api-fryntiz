...
/**
 * RelationManager para los generadores de energía asociados a un config de energía.
 */
class PowerGeneratorsRelationManager extends RelationManager
{
    ...
    public function form(Schema $schema): Schema
    {
        return $schema->components([
            ...
            TextInput::make('battery_temperature')->numeric()->step(0.1)->suffix(' °C')->label('Temp. batería'),
            TextInput::make('battery_percentage')->numeric()->minValue(0)->maxValue(100)->suffix(' %')->label('Batería %'),
            TextInput::make('charging_status')->numeric()->label('Estado carga'),
            TextInput::make('charging_status_label')->maxLength(255)->label('Etiqueta estado'),
            Toggle::make('light_status')->label('Luz activa'),
            TextInput::make('light_brightness')->numeric()->label('Brillo luz'),
        ]);
    }
    ...
    public function table(Table $table): Table
    {
        return $table->columns([
            ...
            TextColumn::make('battery_percentage')->suffix(' %')
                ...
                ->label('Batería'),
            TextColumn::make('battery_temperature')->suffix(' °C')->toggleable()->label('Temp. bat.'),
            TextColumn::make('charging_status_label')->badge()->toggleable()->label('Estado'),
        ])
