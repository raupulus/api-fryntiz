...
- **Dispositivos IoT:** token Sanctum **por dispositivo** con **abilities/scopes**
  ...
  Emisión: `php artisan iot:device-token <id> --abilities=... [--expires=días]`.
- **Endpoints de escritura IoT:** protégelos con la cadena
  `auth:sanctum` + `ability:<scope>` + `throttle:api-store`.
- Aliases de middleware (`bootstrap/app.php`): `cors`, `cors.allow.all`
  (permisivo para IoT/dev), `check.domain`, `ip.counter.strict`, `ability`/`abilities`.
...
6. Test en `tests/Feature/Api/V2/<Modulo>Test.php` (extiende `ApiTestCase`,
   usa `RefreshDatabase`).
7. Actualiza `docs/info/<modulo>.md` y, si aplica, `docs/api-v2.md`.
