...
| Helpers con namespace | `app/Helpers/` | `TextFormatParseHelper`, etc. |
| Traits compartidos | `app/Traits/` | Reutilizar antes de duplicar |

## Modelos

Extiende siempre `BaseModel` (`app/Models/BaseModels/BaseModel.php`). No
extiendas `Illuminate\Database\Eloquent\Model` directamente.
...
Roles (`UserRoleEnum`): `1=SuperAdmin, 2=Admin, 3=User, 4=Editor`. El panel
Admin de Filament acepta SuperAdmin, Admin **y** Editor
(`User::canAccessPanel()`); solo el SuperAdmin tiene el bypass total de
`Gate::before` (`AppServiceProvider`). No hay `RoleHelper`: las comprobaciones
de rol viven en los métodos del propio modelo (`isSuperAdmin()`, `isAdmin()`,
`isEditor()`).
