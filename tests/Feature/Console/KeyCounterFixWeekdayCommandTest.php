...
class KeyCounterFixWeekdayCommandTest extends TestCase
{
    ...
    /**
     * @param  string  $cuando  Fecha de la racha.
     * @param  int  $weekday  Valor tal cual está guardado.
     */
    private function racha(string $cuando, int $weekday): Keyboard
    {
    ...
    public function en_seco_no_escribe_nada(): void
    {
        ...
        $racha = $this->racha('2019-12-28 14:00:00', 6);

        $this->artisan('keycounter:fix_weekday')
            ->expectsOutputToContain('Modo seco')
            ->assertSuccessful();

        $this->assertSame(6, (int) $racha->refresh()->weekday);
    }

    #[Test]
    public function con_write_normaliza_las_rachas_antiguas(): void
    {
        ...
        $this->assertSame(5, (int) $racha->refresh()->weekday);
    }

    #[Test]
    public function no_toca_nada_posterior_al_cambio_de_convencion(): void
    {
        // 2020-03-04 fue miércoles. Con 0=lunes es 2, que es lo correcto.
        $moderna = $this->racha('2020-03-04 10:00:00', 2);

        $this->artisan('keycounter:fix_weekday --write')->assertSuccessful();

        $this->assertSame(2, (int) $moderna->refresh()->weekday);
    }
    ...
    public function respeta_las_rachas_que_no_siguen_la_convencion_vieja(): void
    {
        ...
        $medianoche = $this->racha('2019-12-28 23:50:00', 0);

        $this->artisan('keycounter:fix_weekday --write')->assertSuccessful();

        $this->assertSame(5, (int) $yaBuena->refresh()->weekday);
        $this->assertSame(0, (int) $medianoche->refresh()->weekday);
    }

    #[Test]
    public function el_limite_de_fecha_se_puede_mover(): void
    {
        $racha = $this->racha('2019-12-28 14:00:00', 6);
